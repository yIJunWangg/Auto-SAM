...
def train(args, epoch, model, sam_model, dataloader, optimizer, scheduler, training):
    ...
    for idx, batch in tqdm.tqdm(enumerate(dataloader)):
        ...
        _,vit_features = sam_model.get_feat_from_np(
            batch['query_img'], 
            batch['query_name'],
            batch['support_imgs'].squeeze(1),
        )
        protos = model(args.condition, batch['query_img'], batch['support_imgs'].squeeze(1), batch['support_masks'].squeeze(1), training,vit_query_feat=vit_features[0],vit_supp_feat=vit_features[1])
...
if __name__ == '__main__':
    ...
    sam_model.to(device)
    model.to(device)
    check_point=r'logs/train_0328_142530.log/best_model.pt'
    model.load_state_dict(torch.load(check_point,weights_only=True),strict=False)
    check_point_sam = torch.load(r'logs/train_0328_142530.log/best_model_sam.pt',weights_only=True)
    ...
    for epoch in range(args.epochs):
        trn_loss, trn_miou, trn_fb_iou ,trn_mPA = train(args, epoch, model, sam_model, dataloader_trn, optimizer, scheduler, training=True)
        with torch.no_grad():
            val_loss, val_miou, val_fb_iou ,val_mPA = train(args, epoch, model, sam_model, dataloader_val, optimizer, scheduler, training=False)
