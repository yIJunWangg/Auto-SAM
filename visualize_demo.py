...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_dir', type=str, default='results/demo', help='Directory to save prediction masks')
    ...
    sam_model.to(device)
    model.to(device)
    check_point=r'logs/train_0401_112338.log/best_model.pt'
    model.load_state_dict(torch.load(check_point,weights_only=True))
    check_point_sam = torch.load(r'logs/train_0401_112338.log/best_model_sam.pt',weights_only=True)
    ...
    for epoch in range(args.epochs):#测试
        with torch.no_grad():
            ...
            val_loss, val_miou, val_fb_iou, mPA = visualize(args, epoch, model, sam_model, dataloader_val, training=False,dataroot='dataset')
